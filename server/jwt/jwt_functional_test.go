...
func TestJWTValidationWithIncludedPathInDomainConfig(t *testing.T) {
...
func TestJWTValidationWithIncludedPathInCommonConfigWithDomain(t *testing.T) {
	config.Config = getCommonConfig()
	config.Config.Domains = make(config.Domains)
	initConfig()
	req, err := getRequest()
...
func TestJWTValidationWithIncludedPathInCommonConfigWithoutDomain(t *testing.T) {
	config.Config = getCommonConfig()
	initConfig()
	req, err := getRequest()
	assert.Nil(t, err)
	...
	assert.Equal(t, http.StatusUnauthorized, rr.Code)
	assert.Containsf(t, rr.Body.String(), "failed to fetch resource pointed by", "No jwks url provided status code should be 401")
	tearDownHTTPFunctional()
}
